...
function index(req, res) {
    const sql = 'SELECT films.*, AVG(reviews.average_rating) AS avg FROM films JOIN reviews ON reviews.film_id = films.id GROUP BY films.id'
...
function show(req, res) {
    const sql = 'SELECT films.*, AVG(reviews.average_rating) AS avg FROM films JOIN reviews ON reviews.film_id = films.id WHERE films.id = ? GROUP BY films.id '
